...
;; (superproject com-junkawasaki/root の manifest/edn-datomize.bb から移植・adapt —
;;  この child repo には manifest/ が無いので schema.edn は repo root に置く。)
;;
...
;;
;; 使い方 (network-isekai 用に schema-path を repo root/schema.edn に adapt 済み):
;;   bb scripts/edn-datomize.bb wrap-map <path> <ns>              — map 1個のファイルを変換
;;   bb scripts/edn-datomize.bb wrap-map-preserve <path> <def-ns> — 既存の namespaced key は
;;                                                                    温存し、bare key だけ
;;                                                                    def-ns を付けて昇格
...
;; com-google-ads has no manifest/ dir (that's a superproject convention) — schema.edn
;; lives at this child repo's own root instead.
...
;; ---------- namespace-preserving mode (Phase 2 kotoba-lang/security & net-kotobase
;; pilot pattern) ----------
;;
;; For files whose keys are ALREADY meaningfully namespaced (idiomatic Clojure style,
;; e.g. :concept/id, :game/title), blindly re-prefixing with a new file-level namespace
;; would double-namespace / destroy the existing meaning. Only genuinely bare
;; (unnamespaced) keys get promoted to default-ns; already-namespaced keys pass through
;; unchanged.

(defn namespaced-key-preserving [default-ns k]
  (if (namespace k) k (keyword default-ns (name k))))

(defn entity-from-map-preserving [content default-ns]
  (into {:db/id -1}
        (map (fn [[k v]] [(namespaced-key-preserving default-ns k) (attr-value v)]))
        content))

(defn schema-attrs-preserving [content default-ns]
  (for [[k v] content]
    (let [{:keys [type card]} (classify v)]
      {:db/ident (namespaced-key-preserving default-ns k)
       :db/valueType type
       :db/cardinality card})))

(defn wrap-map-preserve! [rel-path default-ns]
  (let [f (io/file root rel-path)
        content (slurp-edn f)]
    (if (already-tx-data? content)
      (println "skip (already tx-data):" rel-path)
      (let [entity (entity-from-map-preserving content default-ns)
            attrs (schema-attrs-preserving content default-ns)]
        (spit f (pr-str [entity]))
        (merge-schema! attrs)
        (println "wrapped(preserve-ns)" rel-path "->" (count entity) "attrs, default-ns=" default-ns)))))
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb scripts/edn-datomize.bb [wrap-map <path> <ns> | wrap-map-preserve <path> <default-ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
